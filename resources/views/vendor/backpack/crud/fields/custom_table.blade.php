...
@include('crud::fields.inc.wrapper_start')
    ...
    <label class="answers">{!! $field['label'] !!}</label>
    @include('crud::fields.inc.translatable_icon')

    <input class="array-json"
            type="hidden"
            data-init-function="bpFieldInitTableElementCustom"
            name="{{ $field['name'] }}"
            value="{{ $items }}"
            data-max="{{$max}}"
            data-min="{{$min}}"
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-sm table-striped m-b-0 answers">
            ...
            <tbody class="table-striped items sortableOptions">
                ...
                <tr class="array-row clonable" style="display: none;">
                    ...
                    @foreach( $field['columns'] as $column => $item)
                   ...
                   @if($item['type']=="text")
                    <td>
                       ...
                       @if($item['type']=="radio")
                           <td>
                               <input class="form-control form-control-sm correct-radio" name="item.correct" value=0 type="radio" data-cell-name="item.{{ $column }}">
                           </td>
                       @endif
        ...
        <div class="array-controls btn-group m-t-10">
            <button class="btn btn-sm btn-light answers" type="button" data-button-type="addItem"><i class="la la-plus"></i> {{trans('backpack::crud.add')}} {{ $item_name }}</button>
        </div>
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            $(document).on('change', '.que-type', function (event) {
                event.preventDefault();
                var queType = $(this).val();
                if(queType == "text")
                {
                    $(".answers").hide();
                    $(".la-flag-checkered").hide();
                }
                else{
                    $(".answers").show();
                    $(".la-flag-checkered").show();
                }

            });

            $(document).ready(function() {
                var queType = $(".que-type").val();
                console.log(queType);
                if(queType == "text")
                {
                    $(".answers").hide();
            ...
            function bpFieldInitTableElementCustom(element) {
                ...
                $(document).on('change', '.correct-radio', function (event) {
                    $(".correct-radio").val(0);
                    $(this).val(1);
                ...
                $tableWrapper.find('.correct-radio').on('change', function() {
                    $(".correct-radio").val(0);
                    $(this).val(1);
